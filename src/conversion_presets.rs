...
pub struct TargetCompatibilityKey {
    ...
    /// Required output-profile hash for ICC mode, DeviceLink hash for DeviceLink
    /// mode, or None when the selected engine does not use that profile class.
    #[serde(default)]
    pub transform_sha256: Option<String>,
    /// Required measured-characterization identity for Custom Optimizer mode.
    #[serde(default)]
