...
pub enum CharacterizedCandidateError {
    /// Retained for API compatibility; now means the target has neither a
    /// measured characterization identity nor an Output ICC identity.
    MissingTargetCharacterization,
    /// Retained for API compatibility; `target` lists all accepted forward-model
    /// identities when more than one authority is available.
...
/// Build a separation candidate from an authoritative forward color model
/// instead of accepting caller-supplied color error or neutrality data.
///
/// The model may be backed by measured characterization or by the exact
/// identity-verified target Output ICC's device→PCS transform. Exact channel
/// order must match in both cases. Neutral classification uses the requested PCS
/// color's chroma, so an ink vector cannot make itself eligible for the
/// neutral-Black reward.
pub fn characterize_candidate(
    ...
    if authorities.is_empty() {
        return Err(CharacterizedCandidateError::MissingTargetCharacterization);
    }
...
mod tests {
    ...
    #[test]
    fn characterized_candidate_accepts_output_icc_identity_without_measurement() {
