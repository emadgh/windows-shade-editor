...
mod tests {
    ...
    fn png_runtime_view_keeps_alpha_outside_channel_topology() {
        ...
        assert_eq!(RuntimePreviewSource::color_model(&preview), RuntimeColorModel::Rgb);
        assert_eq!(RuntimePreviewSource::channels(&preview).len(), 3);
        assert_eq!(RuntimePreviewSource::channel_names(&preview), ["Red", "Green", "Blue"]);
        assert_eq!(RuntimePreviewSource::alpha(&preview), Some(&[400][..]));
        assert_eq!(RuntimePreviewSource::embedded_icc(&preview), Some(&[1, 2, 3][..]));
