...
pub fn default_converted_filename(source: &Path, target_suffix: &str) -> Result<PathBuf, OutputPathError> {
    let stem = source
        .file_stem()
        .and_then(|value| value.to_str())
        .ok_or(OutputPathError::MissingFileName)?;
    let suffix = sanitize_suffix(target_suffix);
    let suffix = if suffix.is_empty() { "converted" } else { suffix.as_str() };
...
mod tests {
    ...
    fn production_output_is_tiff_only() {
        ...
        assert!(validate_conversion_output_path(source, Path::new(r"C:\Production\Face01_7C.tif")).is_ok());
        assert_eq!(
