...
fn draw_test_code(
    ...
    let bytes = fs::read(&font_path).map_err(|err| format!("Cannot read {}: {err}", font_path.display()))?;
    ...
    if bitmap.width == 0 || bitmap.height == 0 { return Ok(()); }
    ...
    let x0 = match position {
        ...
        TestCodePosition::TopRight | TestCodePosition::BottomRight => width.saturating_sub(margin_x.saturating_add(bitmap.width)),
    };
    let y0 = match position {
        TestCodePosition::TopLeft | TestCodePosition::TopRight => margin_y,
        TestCodePosition::BottomLeft | TestCodePosition::BottomRight => height.saturating_sub(margin_y.saturating_add(bitmap.height)),
    };
...
fn rasterize_text(font: &Font, text: &str, px: f32) -> TextBitmap {
    ...
    let height = lines.len().max(1).saturating_mul(line_height).saturating_add(4);
    ...
    for (line_index, line) in lines.iter().enumerate() {
        ...
        for ch in line.chars() {
            ...
            let baseline_bottom = (line_index * line_height + line_height).saturating_sub(2) as isize;
