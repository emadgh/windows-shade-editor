...
use windows_shade_editor::characterization_intake::{
    CharacterizationIntakeMetadata, CharacterizationIntakeResult,
    MeasurementCoverageUnit, MeasurementTableDelimiter,
...
impl Default for CharacterizationIntakeUiState {
    fn default() -> Self {
        Self {
            ...
            channel_limits: vec![String::new(); 4],
...
pub(crate) fn render_characterization_intake(
    ...
    ui.horizontal_wrapped(|ui| {
        ui.label("Delimiter");
        invalidate |= ui
            .selectable_value(&mut state.delimiter, MeasurementTableDelimiter::Comma, "CSV comma")
            .changed();
        invalidate |= ui
            .selectable_value(&mut state.delimiter, MeasurementTableDelimiter::Tab, "TSV tab")
            .changed();
        ui.separator();
    ...
    if !state.table_text.is_empty() {
        if let Some(header) = state.table_text.lines().find(|line| !line.trim().is_empty()) {
    ...
    egui::Grid::new("characterization-intake-package-metadata")
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                invalidate |= ui.selectable_value(&mut state.output_bit_depth, 8, "8-bit").changed();
                invalidate |= ui.selectable_value(&mut state.output_bit_depth, 16, "16-bit").changed();
            });
            ui.end_row();
        });
    ui.label(
        egui::RichText::new(
            "Selecting ProductionValidated only declares the package metadata. It does not approve representativeness, thresholds, or Custom Optimizer production use.",
        )
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        if state.channel_names.len() > 4 && ui.small_button("Remove last").clicked() {
            state.channel_names.pop();
            state.channel_limits.pop();
            invalidate = true;
        }
        if state.channel_names.len() < 12 && ui.small_button("Add channel").clicked() {
    ...
    egui::Grid::new("characterization-intake-channels")
        ...
        .show(ui, |ui| {
            ...
            for index in 0..state.channel_names.len() {
                ...
                invalidate |= ui.text_edit_singleline(&mut state.channel_names[index]).changed();
                invalidate |= ui.text_edit_singleline(&mut state.channel_limits[index]).changed();
                ui.end_row();
            }
        });
    ui.horizontal(|ui| {
        ui.label("Measured total-ink limit");
        invalidate |= ui.text_edit_singleline(&mut state.total_ink_limit).changed();
    });
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        let save_enabled = state.built.is_some();
        if ui
            .add_enabled(save_enabled, egui::Button::new("Save package JSON..."))
            .clicked()
        {
...
fn build_package(state: &mut CharacterizationIntakeUiState) {
    ...
    if state.table_text.trim().is_empty() {
        state.errors.push("Load a CSV/TSV measurement table first.".to_owned());
        return;
    }
...
fn save_package_dialog(state: &mut CharacterizationIntakeUiState) {
    ...
    let file_name = format!("{}.characterization.json", safe_filename_component(&result.package.payload.revision));
    let Some(path) = rfd::FileDialog::new()
    ...
    match save_characterization_package(&path, &result.package) {
        Ok(()) => {
            state.errors.clear();
            state.status = Some(format!("Saved measured characterization package: {}", path.display()));
        }
        Err(error) => {
            state.errors = vec![error];
            state.status = None;
...
fn invalidate_built_result(state: &mut CharacterizationIntakeUiState) {
    if state.built.take().is_some() {
        state.status = Some("Inputs changed; validate and build the package again before saving.".to_owned());
    }
}
...
mod tests {
    ...
    fn any_input_change_invalidates_a_built_result_before_save() {
        ...
        assert!(source.contains("add_enabled(save_enabled"));
    }
