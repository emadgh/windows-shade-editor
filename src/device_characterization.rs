...
impl LabColor {
    /// CIE L*C*h chroma. Neutral classification for separation strategy must be
    /// based on the requested PCS color, not on the chosen ink vector.
...
pub struct CharacterizationIdentity {
    /// Stable target-owned characterization identity/version.
    pub id: String,
...
/// target-ink vector. Production implementations must be backed by measured or
/// otherwise validated characterization data. UI display colors are never a
/// valid device model.
...
pub fn evaluate_characterized_color(
    ...
    if model.identity().id.trim().is_empty() {
        return Err("Characterization identity is empty.".to_owned());
    }
    if model.identity().channel_names.is_empty() {
        return Err("Characterization has no production channels.".to_owned());
    }
    if model
        .identity()
        .channel_names
    ...
    {
        return Err("Characterization contains an empty production channel name.".to_owned());
    }
    ...
    if !lab_is_finite(target_lab) {
        return Err("Characterization received non-finite target Lab values.".to_owned());
    }

    let predicted = model.predict_lab(coverages)?;
    if !lab_is_finite(predicted) {
        return Err("Characterization produced non-finite Lab values.".to_owned());
    }
...
mod tests {
    ...
    impl DeviceForwardModel for SyntheticFourInkModel {
        ...
        fn predict_lab(&self, coverages: &[f32]) -> Result<LabColor, String> {
            ...
            // Test-only deterministic model. Production implementations must use
    ...
    #[test]
    fn lab_chroma_is_finite_for_valid_target() {
        let color = LabColor {
            l: 50.0,
            a: 3.0,
            b: 4.0,
        };
        assert_eq!(color.chroma(), 5.0);
    }
}
