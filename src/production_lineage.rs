...
fn inspect_lineage(path: PathBuf, source_project_path: &Path) -> LinkedProductionLineage {
    ...
    if !project.linked_projects.iter().any(|link| {
        link.role == ProjectRole::Source
            && paths_match(
                &resolve_linked_path(&link.path, &path),
                source_project_path,
            )
    }) {
        return failed(
...
fn resolve_linked_path(recorded: &str, project_path: &Path) -> PathBuf {
...
mod tests {
    ...
    #[test]
    fn relative_reciprocal_source_link_resolves_from_production_directory() {
        let source = temp_path("relative-source", "shade");
        let face = temp_path("relative-face", "tif");
        let output = temp_path("relative-output", "tif");
        let production_path = temp_path("relative-production", "shade");
        let mut production = build_production_project(ProductionProjectSpec {
            project_name: "Production",
            source_project_path: &source,
            output_tiff_path: &output,
            output_face_label: "Face 1",
        ...
        let source_name = source
            .file_name()
            .expect("Source fixture has a filename")
            .to_string_lossy()
            .into_owned();
        let source_link = production
            .linked_projects
            .iter_mut()
            .find(|link| link.role == ProjectRole::Source)
            .expect("Production fixture has Source backlink");
        source_link.path = source_name;
        production
            .save_new(&production_path, std::slice::from_ref(&output))
            .unwrap();
