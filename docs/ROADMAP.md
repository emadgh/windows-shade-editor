...
Implemented foundations: Source/Production architecture contracts and persisted lineage/provenance, saved-source gate, shared production preflight, separate per-Face Production Source ICC assignment, Output ICC/DeviceLink Target Setup with verified profile identity/topology/channel order, safe TIFF destination binding, RGB→CMYK and CMYK→CMYK LittleCMS transforms, typed 5C–12C ICC transforms, direct single-profile DeviceLink execution, capability/preset compatibility, constrained Black-focused candidate ranking, deterministic recipe identity, non-destructive output-path policy, PNG source decoding, an atomic LZW CMYK/true-Separated-N-ink TIFF writer with strip-bounded transforms, a local disk-backed mmap compression boundary, truthful optional output ICC and standard ink tags, strict N-ink reopen validation with direct coverage semantics, an immutable conversion-job/transaction/recovery contract, a real production raster backend that revalidates exact input/profile hashes and uses a local mmap spool, and a persistent queue/UI with explicit restart resume, pause/cancel/retry/recovery plus hash-guarded reciprocal Source linking.

Next production path: add deterministic real RGB→CMYK and N-channel Output-ICC fixtures, define Photoshop metadata only for an output representation that actually uses Spot-style channels, and run real Photoshop/RIP validation of true Separated N-ink output. Sources containing Spot/extra samples and non-streamable sources remain explicitly rejected by the production backend. Reusable target presets and Custom N-ink candidate generation remain pending; the custom path is blocked on characterized target data because channel display colors are not a device model.
