...
`src/conversion_tiff.rs` provides the output boundary for 8/16-bit CMYK and 5C–12C conversion samples. It requests one strip at a time from the conversion worker, selects classic TIFF or BigTIFF from the estimated raw size (or an explicit override), embeds the exact target ICC and writes `InkSet`, NUL-separated `InkNames`, `NumberOfInks`, DPI and orientation. Four-channel output is ordinary CMYK (`InkSet=1`). Non-CMYK 5C–12C output uses Photometric Separated with `InkSet=2`, all N inks represented as base samples and no `ExtraSamples`; samples retain the transform's direct 0=no ink coverage polarity.
...
The reader recognizes non-CMYK Separated output only when Photometric=Separated, `InkSet=2`, `NumberOfInks=SamplesPerPixel`, exact NUL-separated `InkNames`, consistent per-ink bit depth and no `ExtraSamples` agree. Ambiguous declarations are rejected instead of being reopened as CMYK. Because image-tiff does not directly decode arbitrary Separated sample counts, Shade Editor applies its existing read-only Photometric overlay only inside the decoder view; file bytes and the retained metadata remain unchanged.

The encoder's compressed path requires a complete image slice, so the writer renders callback strips into a uniquely-created local disk spool and memory-maps that file read-only for final LZW encoding. This keeps transformation memory bounded without allocating a second full-resolution N-channel image, verifies Compression=5 in the staged TIFF and removes the spool on success or failure. Photoshop Image Resources/DisplayInfo generation for representations that actually use Spot channels, external Photoshop/RIP round trips and representative >4 GiB BigTIFF validation remain mandatory before claiming production interchange compatibility.
