...
# Distinguish this follow-up build from the previous 0.21.26 milestone build.
replace_count("Cargo.toml", 'version = "0.21.26"', 'version = "0.21.27"')
version_path = Path("VERSION")
if version_path.read_text(encoding="utf-8").strip() != "0.21.26":
    raise SystemExit("VERSION: expected 0.21.26")
version_path.write_text("0.21.27\n", encoding="utf-8", newline="\n")

lock_path = Path("Cargo.lock")
lock_text = lock_path.read_text(encoding="utf-8")
pattern = r'(name = "windows-shade-editor"\nversion = ")0\.21\.26("\n)'
lock_text, count = re.subn(pattern, r'\g<1>0.21.27\g<2>', lock_text, count=1)
if count != 1:
