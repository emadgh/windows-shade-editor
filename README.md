...
- The production raster backend revalidates source/profile hashes, renders the exact saved adjustments into a local mmap spool, converts pure RGB/CMYK TIFF sources through Standard Output ICC or a direct DeviceLink to CMYK or 5C–12C, and feeds bounded 8/16-bit strips into the verified atomic writer.
- Valid Standard Output ICC and DeviceLink recipes are captured in the background and run through a persistent Conversion Queue with restart-safe explicit resume, phase progress, pause, pre-commit cancellation, retry and durable recovery state.
...
- Production conversion output is LZW-compressed through a local disk-backed mmap spool, preserving strip-bounded transform RAM and atomic staged verification/commit. Non-CMYK 5C–12C output is encoded as a true Separated N-ink space (`InkSet=2`, N base samples, no `ExtraSamples`) and reopens with direct coverage plus authoritative `InkNames`. Deterministic real RGB→CMYK/N-channel Output-ICC fixtures, Spot/extra-sample sources, non-streamable sources, Photoshop-specific metadata and real RIP acceptance remain validation gates.
