...
if 'version = "0.12.0"' in cargo:
    cargo = cargo.replace('version = "0.12.0"', 'version = "0.12.1"', 1)
elif 'version = "0.12.1"' not in cargo:
    raise RuntimeError("unexpected Cargo version")
write("Cargo.toml", cargo)

settings = read("src/settings_v6.rs")
settings = replace_once(settings, "    pub validate_after_export: bool,\n    pub default_dpi: f64,", "    pub validate_after_export: bool,\n    pub lzw_compression: bool,\n    pub default_dpi: f64,", "settings LZW field")
settings = replace_once(settings, "            validate_after_export: false,\n            default_dpi: DEFAULT_DPI,", "            validate_after_export: false,\n            lzw_compression: true,\n            default_dpi: DEFAULT_DPI,", "settings LZW default")
settings = replace_once(settings, "    fn post_export_validation_defaults_off() {\n        assert!(!AppSettings::default().validate_after_export);\n    }\n", "    fn post_export_validation_defaults_off() {\n        assert!(!AppSettings::default().validate_after_export);\n    }\n\n    #[test]\n    fn lzw_compression_defaults_on() {\n        assert!(AppSettings::default().lzw_compression);\n    }\n", "settings LZW test")
write("src/settings_v6.rs", settings)

export = read("src/export_v6.rs")
export = replace_once(export, "use crate::tiff_io::{\n    ColorModel, StreamInfo, TiffMetadata, decode_full, for_each_decoded_region,\n    for_each_decoded_strip, stream_info,\n};\n\npub fn export_face(", "use crate::tiff_io::{\n    ColorModel, StreamInfo, TiffMetadata, decode_full, for_each_decoded_region,\n    for_each_decoded_strip, stream_info,\n};\n\n#[derive(Clone, Copy, Debug)]\npub struct ExportOptions {\n    pub force_lzw: bool,\n}\n\nimpl Default for ExportOptions {\n    fn default() -> Self {\n        Self { force_lzw: true }\n    }\n}\n\npub fn export_face(", "export options type")
export = regex_once(export, r"pub fn export_face_with_progress<F>\(.*?\n\}\n\nfn export_face_direct_with_progress<F>", '''pub fn export_face_with_progress<F>(
    source: &Path,
    destination: &Path,
...
fn export_face_direct_with_progress<F>''', "export progress options wrapper")
export = replace_once(export, "    project: &ShadeProject,\n    default_dpi: f64,\n    mut progress: F,\n) -> Result<(), String>\nwhere\n    F: FnMut(f32, &str),\n{\n    progress(0.02, \"Inspecting TIFF\");", "    project: &ShadeProject,\n    default_dpi: f64,\n    options: ExportOptions,\n    mut progress: F,\n) -> Result<(), String>\nwhere\n    F: FnMut(f32, &str),\n{\n    progress(0.02, \"Inspecting TIFF\");", "direct export options argument")
export = replace_once(export, "            project,\n            default_dpi,\n            &stream,\n            &mut progress,", "            project,\n            default_dpi,\n            options,\n            &stream,\n            &mut progress,", "streaming call options")
export = export.replace("                dpi_info,\n                None,\n                OutputPixels::", "                dpi_info,\n                options,\n                None,\n                OutputPixels::")
export = replace_once(export, "    project: &ShadeProject,\n    default_dpi: f64,\n    stream: &StreamInfo,", "    project: &ShadeProject,\n    default_dpi: f64,\n    options: ExportOptions,\n    stream: &StreamInfo,", "stream export options argument")
export = export.replace("                    dpi_info,\n                    Some(stream.rows_per_strip),\n                    OutputPixels::", "                    dpi_info,\n                    options,\n                    Some(stream.rows_per_strip),\n                    OutputPixels::")
export = replace_once(export, "fn configure_tiff_encoder<W, K>(\n    mut encoder: TiffEncoder<W, K>,\n    metadata: &TiffMetadata,\n) -> TiffEncoder<W, K>", "fn configure_tiff_encoder<W, K>(\n    mut encoder: TiffEncoder<W, K>,\n    metadata: &TiffMetadata,\n    options: ExportOptions,\n) -> TiffEncoder<W, K>", "configure encoder options argument")
export = replace_once(export, "    let compression = match metadata.compression {\n        Some(1) => Compression::Uncompressed,\n        Some(5) => Compression::Lzw,\n        Some(8 | 32946) => Compression::Deflate(tiff::encoder::DeflateLevel::Balanced),\n        Some(32773) => Compression::Packbits,\n        _ => Compression::Lzw,\n    };", "    let compression = if options.force_lzw {\n        Compression::Lzw\n    } else {\n        match metadata.compression {\n            Some(1) => Compression::Uncompressed,\n            Some(5) => Compression::Lzw,\n            Some(8 | 32946) => Compression::Deflate(tiff::encoder::DeflateLevel::Balanced),\n            Some(32773) => Compression::Packbits,\n            _ => Compression::Lzw,\n        }\n    };", "LZW compression policy")
export = replace_once(export, "    metadata: &TiffMetadata,\n    dpi_info: DpiInfo,\n    rows_per_strip: Option<u32>,", "    metadata: &TiffMetadata,\n    dpi_info: DpiInfo,\n    options: ExportOptions,\n    rows_per_strip: Option<u32>,", "write TIFF options argument")
export = export.replace("configure_tiff_encoder(encoder, metadata);", "configure_tiff_encoder(encoder, metadata, options);")
write("src/export_v6.rs", export)

validation = read("src/validation.rs")
validation = regex_once(validation, r"pub fn validate_no_adjustment_roundtrip<F>\(.*?\n\{\n    if !source\.is_file\(\) \{", '''pub fn validate_no_adjustment_roundtrip<F>(
    source: &Path,
    output_folder: &Path,
    default_dpi: f64,
    progress: F,
...
where
    F: FnMut(f32, &str),
{
    if !source.is_file() {''', "validation options wrapper")
validation = replace_once(validation, "    export::export_face_with_progress(\n        source,\n        &export_path,\n        &identity_project,\n        default_dpi,\n        |fraction, detail| progress(0.12 + fraction * 0.58, detail),\n    )?;", "    export::export_face_with_progress_options(\n        source,\n        &export_path,\n        &identity_project,\n        default_dpi,\n        export::ExportOptions { force_lzw },\n        |fraction, detail| progress(0.12 + fraction * 0.58, detail),\n    )?;", "validation export options")
validation = validation.replace("expected_export_compression(source_decoded.metadata.compression)", "expected_export_compression(source_decoded.metadata.compression, force_lzw)", 1)
validation = replace_once(validation, "pub fn validate_export_transport(source: &Path, exported: &Path) -> Result<String, String> {\n    let source_info =", "pub fn validate_export_transport(source: &Path, exported: &Path) -> Result<String, String> {\n    validate_export_transport_with_options(source, exported, true)\n}\n\npub fn validate_export_transport_with_options(\n    source: &Path,\n    exported: &Path,\n    force_lzw: bool,\n) -> Result<String, String> {\n    let source_info =", "transport validation options wrapper")
validation = validation.replace("expected_export_compression(source_meta.compression)", "expected_export_compression(source_meta.compression, force_lzw)", 1)
validation = replace_once(validation, "fn expected_export_compression(source: Option<u16>) -> Option<u16> {\n    match source {\n        Some(1 | 5 | 8 | 32946 | 32773) => source,\n        _ => Some(5),\n    }\n}", "fn expected_export_compression(source: Option<u16>, force_lzw: bool) -> Option<u16> {\n    if force_lzw {\n        return Some(5);\n    }\n    match source {\n        Some(1 | 5 | 8 | 32946 | 32773) => source,\n        _ => Some(5),\n    }\n}", "expected compression policy")
write("src/validation.rs", validation)

app = read("src/app_main.rs")
app = replace_once(app, "const ERROR_TOAST_LIFETIME: Duration = Duration::from_secs(120);", "const ERROR_TOAST_LIFETIME: Duration = Duration::from_secs(8);", "error toast lifetime")
app = replace_once(app, "        self.log.error(&message);\n        self.status_message = message.clone();\n        self.toast = Some(ErrorToast {", "        self.log.error(&message);\n        self.status_message = \"Error - see Logs\".to_owned();\n        self.toast = Some(ErrorToast {", "error status message")
app = replace_once(app, "        if self\n            .toast\n            .as_ref()\n            .is_some_and(|toast| toast.created.elapsed() > ERROR_TOAST_LIFETIME)\n        {\n            self.toast = None;\n        }", "        if self\n            .toast\n            .as_ref()\n            .is_some_and(|toast| toast.created.elapsed() > ERROR_TOAST_LIFETIME)\n        {\n            self.toast = None;\n            if self.status_message == \"Error - see Logs\" {\n                self.status_message = \"Ready\".to_owned();\n            }\n        }", "error status expiration")

app = replace_once(app, "        let default_dpi = self.settings.default_dpi;\n        let validate_after_export = self.settings.validate_after_export;\n        self.launch_job(\"Exporting TIFF\"", "        let default_dpi = self.settings.default_dpi;\n        let force_lzw = self.settings.lzw_compression;\n        let validate_after_export = self.settings.validate_after_export;\n        self.launch_job(\"Exporting TIFF\"", "current export LZW setting")
app = replace_once(app, "            let result = export::export_face_with_progress(\n                &source,\n                &destination,\n                &project,\n                default_dpi,", "            let result = export::export_face_with_progress_options(\n                &source,\n                &destination,\n                &project,\n                default_dpi,\n                export::ExportOptions { force_lzw },", "current export options call")
app = replace_once(app, "                    let verified = validation::validate_export_transport(&source, &destination)?;", "                    let verified = validation::validate_export_transport_with_options(\n                        &source,\n                        &destination,\n                        force_lzw,\n                    )?;", "current export validation policy")
app = replace_once(app, "        let source = face.path.clone();\n        let default_dpi = self.settings.default_dpi;\n        self.launch_job(\"Validating TIFF\"", "        let source = face.path.clone();\n        let default_dpi = self.settings.default_dpi;\n        let force_lzw = self.settings.lzw_compression;\n        self.launch_job(\"Validating TIFF\"", "validation LZW setting")
app = replace_once(app, "            let result = validation::validate_no_adjustment_roundtrip(\n                &source,\n                &folder,\n                default_dpi,", "            let result = validation::validate_no_adjustment_roundtrip_with_options(\n                &source,\n                &folder,\n                default_dpi,\n                force_lzw,", "validation options call")
app = replace_once(app, "        let project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        let validate_after_export = self.settings.validate_after_export;\n        self.launch_job(\"Exporting faces\"", "        let project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        let force_lzw = self.settings.lzw_compression;\n        let validate_after_export = self.settings.validate_after_export;\n        self.launch_job(\"Exporting faces\"", "all export LZW setting")
app = replace_once(app, "                    export::export_face_with_progress(\n                        source,\n                        &destination,\n                        &project,\n                        default_dpi,", "                    export::export_face_with_progress_options(\n                        source,\n                        &destination,\n                        &project,\n                        default_dpi,\n                        export::ExportOptions { force_lzw },", "all export options call")
app = replace_once(app, "                        validation::validate_export_transport(source, &destination)?;", "                        validation::validate_export_transport_with_options(\n                            source,\n                            &destination,\n                            force_lzw,\n                        )?;", "all export validation policy")
app = replace_once(app, "        let mut project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        project.adjustments = snapshot.adjustments.clone();", "        let mut project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        let force_lzw = self.settings.lzw_compression;\n        project.adjustments = snapshot.adjustments.clone();", "snapshot export LZW setting")
app = replace_once(app, "            let result = export::export_face_with_progress(\n                &source,\n                &destination,\n                &project,\n                default_dpi,", "            let result = export::export_face_with_progress_options(\n                &source,\n                &destination,\n                &project,\n                default_dpi,\n                export::ExportOptions { force_lzw },", "snapshot export options call")
app = replace_once(app, "        let base_project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        let snapshots = snapshot_ids", "        let base_project = self.project.clone();\n        let default_dpi = self.settings.default_dpi;\n        let force_lzw = self.settings.lzw_compression;\n        let snapshots = snapshot_ids", "snapshot group LZW setting")
app = replace_once(app, "                    export::export_face_with_progress(\n                        &source,\n                        &destination,\n                        &project,\n                        default_dpi,", "                    export::export_face_with_progress_options(\n                        &source,\n                        &destination,\n                        &project,\n                        default_dpi,\n                        export::ExportOptions { force_lzw },", "snapshot group export options call")

app = replace_once(app, "    fn save_settings_quietly(&mut self) {\n        if let Err(err) = self.settings.save() {\n            self.report_error(err);\n        }\n    }\n\n    fn sync_update_state(&mut self) {", '''    fn save_settings_quietly(&mut self) {
        if let Err(err) = self.settings.save() {
            self.report_error(err);
        }
    }
    ...
    fn sync_update_state(&mut self) {''', "shell installer helpers")

app = regex_once(app, r"    fn ui_toolbar\(&mut self, ui: &mut egui::Ui\) \{.*?\n    fn ui_operation_progress\(&self, ui: &mut egui::Ui\) \{.*?\n    \}\n\n    fn ui_update_compact", '''    fn ui_toolbar(&mut self, ui: &mut egui::Ui) {
    ...
    fn ui_update_compact''', "toolbar and progress UI")
...
app = replace_once(app, "                ui.small(\"The max dimension is used when TIFF previews are loaded. Use Rebuild previews to apply a changed value to Faces already open in this project.\");\n                changed |= ui\n                    .checkbox(\n                        &mut self.settings.validate_after_export,", "                ui.small(\"The max dimension is used when TIFF previews are loaded. Use Rebuild previews to apply a changed value to Faces already open in this project.\");\n                ui.separator();\n                ui.heading(\"Export & storage\");\n                changed |= ui\n                    .checkbox(\n                        &mut self.settings.lzw_compression,\n                        \"Use LZW compression for exported TIFF files\",\n                    )\n                    .changed();\n                ui.small(\"LZW is enabled by default. Disable it only when you specifically need to preserve a supported source compression mode.\");\n                changed |= ui\n                    .checkbox(\n                        &mut self.settings.validate_after_export,", "export storage settings section")
app = replace_once(app, "                }\n                ui.separator();\n                ui.heading(\"Editor layout\");", '''                }
                ...
                ui.separator();
                ui.heading("Editor layout");''', "Shell integration settings section")

app = replace_once(app, "        egui::Window::new(\"About Shade Editor\")\n            .open(&mut open)\n            .resizable(false)\n            .show(ctx, |ui| {", "        egui::Window::new(\"About Shade Editor\")\n            .open(&mut open)\n            .resizable(true)\n            .default_width(520.0)\n            .show(ctx, |ui| {", "About window sizing")
app = replace_once(app, "                ui.separator();\n                ui.label(\"Shortcuts: Ctrl+S Save · Ctrl+Shift+S Save As · F Fit · 1-9 channel · S Solo · Ctrl+Enter Update Snapshot · Curve arrows nudge; Shift+Arrow uses larger steps.\");", '''                ui.separator();
                ui.strong("Shortcuts");
...
for rel in ["shell/Install-ShadeEditorShell.ps1", "shell/Uninstall-ShadeEditorShell.ps1"]:
    ...
    if "$version = '0.12.0'" in text:
        write(rel, text.replace("$version = '0.12.0'", "$version = '0.12.1'", 1))
...
if not notes.startswith("# Shade Editor v0.12.1"):
    write("RELEASE_NOTES.md", entry + notes)
